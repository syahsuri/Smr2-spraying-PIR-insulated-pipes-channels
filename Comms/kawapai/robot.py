...
class KawaBot:
	...
	def load_as_file(self, file_location=None):
		...
		if file_location != None:
			...
			file_text = inputfile.read() # Store Kawa-as code from file in local variable
	...
	def get_status(self):
		command = b"status\r\n"
		self.telnet.write(command)
		incomming_status = self.telnet.read_until(b">").decode("ascii")
		print(incomming_status)
		return incomming_status

	def get_kawa_IO(self):
		command = b"IO\r\n"
		self.telnet.write(command)
		IO = self.telnet.read_until(b"\x1b").decode("ascii")
		self.telnet.write(b"\r\n")
		return IO

	# def wait_for_kawa_IO(self):
	# 	command = b"IO\r\n"
	# 	self.telnet.write(command)
	...
	def read_incomming_message(self):
		message = self.telnet.read_until(b">").decode("ascii")
		return message
	
	def wait_for_completion(self):
		#controller prints this line when the program is finished
		self.telnet.read_until(b"Program completed.No = 1\r\n") #kinda ugly but works
		return True
...
class Kawa_status:
		def __init__(self):
			self.mode 				= None
			self.monitor_speed 		= None
			self.program_speed 		= None
			self.Accuracy 			= None
			self.stepper_status 	= None
			self.completed_cycles 	= None
			self.remaining_cycles 	= None


		def set_status(self,status_string):
			# split_string = status_string.split("\r\n")
			self.mode 			 = re.findall('Robot status:\\s+(.*)\\.', status_string)
			self.monitor_speed 	 = float(re.findall('Monitor speed\(%\)\\s+=\\s+([\\d.]+)', status_string)[0])
			self.program_speed 	 = float(re.findall('Program speed\(%\)\\s+ALWAYS =\\s+(\\d+\\.?\\d*)', status_string)[0])
			self.Accuracy   	 = float(re.findall('ALWAYS Accu.\\[mm\\]\\s+=\\s+([\\d.]+)', status_string)[0])
			self.stepper_status   = re.findall('Stepper status:\\s+(.*)\\.', status_string)
			self.completed_cycles = int(re.findall('Completed cycles:\\s+(\\d+)', status_string)[0])
			self.remaining_cycles = int( re.findall('Remaining cycles:\\s+(\\d+)', status_string)[0])
		
		def set_cycle_status(self,status_string):
			self.completed_cycles = int(re.findall('Completed cycles:\\s+(\\d+)', status_string)[0])
			self.remaining_cycles = int( re.findall('Remaining cycles:\\s+(\\d+)', status_string)[0])
...
if __name__ == "__main__":
	robot = KawaBot("192.168.0.5",23)
	IO = robot.get_kawa_IO()
	print(F"{robot.get_kawa_IO()}\n\n\n")
	time.sleep(1)

	# robot.motor_power_on()
	# robot.load_as_file("RoboDK path\Prog_test.pg")

	# robot.AS_command("EXECUTE prog1")

	# while robot_status.set_status(robot.get_status()).completed_cycles == 0:
	# 	time.sleep(0.1)
	# print("Done")
