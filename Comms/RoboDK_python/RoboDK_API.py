...
#Local modules:
import RoboDK_python.SurfacePatternGenerator._spgtools as spg_tools
...
def Kawa_Home(robot):
    ...
    upload_kawa_code(robot,"Comms\RoboDK_python\Process_files\Home.pg")
    print(" Running code")  
    run_code(robot,"Home",speed=10)
    print("homing is busy")
...
def initiate_RDK_workspace(RDK,files_dir):
    ...
    # EOAT.setPoseTool(xyzrpw_2_matrix([-132.566, 10.138, 201.870, -30.27, 12.4, 71.35]))
    EOAT.setPoseTool(xyzrpw_2_matrix([-140.93, 16.87,   218.74,   -30.27,    12.4, 71.35]))
...
def process_traject(RDK,traject_paras,i):
    ...
    MachiningProgEvents = {"CallAction"        : "onPoint",
                           ...
                           "Mcode"             : "M_RunCode(%1)",
                           "RapidSpeed"        : 600,  # rapid speed to move to/from the path
                           "Rounding"          : 40,  # blending radius
                           "RoundingOn"        : 1,
                           "SpindleCCW"        : "",
                           "SpindleCW"         : "",
                           "SpindleRPM"        : "SetRPM(%1)",
