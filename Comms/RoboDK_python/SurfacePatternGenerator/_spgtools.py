...
# Set other constants:
TOL_PROJ_Z = 0.5 #robomath.sqrt(2)  # Tolerance to ignore a point as a ratio (if it falls through a window for example)
# TOL_PROJ_Z = 10
...
def CreatePaths(REF, PART, SIZE_X, SIZE_Y, STEP_X, STEP_Y, REPEAT_TIMES=1, REPEAT_OFFSET=2, cover_all=False, even_distribution=False, continuous=False, angle_triangle_deg=0.0, remove_points_not_on_surface=True):
    ...
    #START Custom function 6-1-2024 Tiemen
    RX_avg = np.round(np.average(np.asarray(projected_lines[0])[:,3]))
    RY_avg = np.round(np.average(np.asarray(projected_lines[0])[:,4]))

    match [RX_avg,RY_avg]:
        case [-1, 0]: projected_lines[0] = [line for line in projected_lines[0] if line[3]<=0]
        case [ 0, 1]: projected_lines[0] = [line for line in projected_lines[0] if line[4]>=0]
        case [ 1, 0]: projected_lines[0] = [line for line in projected_lines[0] if line[3]>=0]
        case [ 0,-1]: projected_lines[0] = [line for line in projected_lines[0] if line[4]<=0]
    ...
    for line in projected_lines[0]:
        if line[5] < -0.1 and line[2] < 300:
            line[5] += (-0.002*line[2]) + 0.6
        
        if RX_avg != 0:
            b = 25 * math.sin((2 * math.pi / 25) * a)
            b1 = 0.25 * math.sin((2 * math.pi / 25) * a)
    ...
    for line, line_projected in zip(lines, projected_lines):
        ...
        for i in range(len(line_projected)):
            ...
            if remove_points_not_on_surface:
                if robomath.distance(pti, pti_proj) <50: #1e-3
...
#---------------------------------------------
def CreateProgram(REF,Settings,Settings2,ProgEvents):
    # Retrieve the reference name
    REF_NAME = REF.Name()
    ...
    curve_follow.setParam("Machining",Settings)
    curve_follow.setParam("ProgEvents",ProgEvents)
    curve_follow.setJoints(Settings2["Start_joints"])
    curve_follow.setPose(Settings2["Tool_offset"])
